...
Component({
  ...
  properties: {
    ...
    isGoodEdit: { type: Boolean, value: false }, // 是否是商品编辑页面
    dragId: { type: String, value: "drag" }, // 页面滚动高度
    deleteIcon: { type: String, value: "" },
    moveIcon: { type: String, value: "" },
    currentDeleteId: { type: Number, value: 0 }, // 当前要删除的id
    child: { type: Boolean, value: false }, // 是否为子类的拖拽列表
  },
  data: {
    /* 未渲染数据 */
  ...
  methods: {
    nameClick(e) {
      ...
      if (this.data.isGoodEdit && currentItem.data) {
        const pages = getCurrentPages();
        const prevPage = pages[pages.length - 2];
        prevPage.setData({
          categoryInfo: currentItem.data,
        });
    ...
    longPress(e) {
      ...
      const { platform, itemDom, itemWrapDom } = this.data;
      const {
        pageX: startPageX,
        pageY: startPageY,
        identifier: startId,
      ...
      const tranY = startPageY - itemDom.height / 2 - itemWrapDom.top;
      // 单列时候X轴初始不做位移
    ...
    touchMove(e) {
      ...
      const tranY = currentPageY - itemDom.height / 2 - itemWrapDom.top;
      // 单列时候X轴初始不做位移
      ...
      if (currentClientY > windowHeight - itemDom.height - realBottomSize) {
        ...
        wx.pageScrollTo({
          scrollTop:
            Number(currentPageY) +
            Number(itemDom.height) -
            (windowHeight - realBottomSize),
          duration: 300,
        });
        // }
      } else if (
        currentClientY <
        Number(itemDom.height) + Number(realTopSize)
      ) {
      ...
      const startKey = parseInt(e.currentTarget.dataset.key, 10);
      const curX = Math.round(tranX / itemDom.width);
      const curY = Math.round(tranY / itemDom.height);
      const endKey = curX + columns * curY;

      // 遇到固定项和超出范围则返回
      if (
        this.isFixed(endKey) ||
        IsOutRange(curX, columns, curY, rows, endKey, this.data.list.length)
      )
        return;
    ...
    excludeFix(sortKey, startKey, type) {
      ...
      if (this.data.list[sortKey].fixed) {
        const _sortKey = type === "reduce" ? sortKey - 1 : Number(sortKey) + 1;
    ...
    triggerCustomEvent(list, type) {
      ...
      this.triggerEvent(type, { listData });
    },
    /**
     *  初始化获取 dom 信息
     */
    initDom() {
      // eslint-disable-next-line no-undef
      const { windowWidth, windowHeight, platform, SDKVersion } =
        wx.getSystemInfoSync();

      this.data.pageMetaSupport = compareVersion(SDKVersion, "2.9.0") >= 0;

      const remScale = (windowWidth || 375) / 375;
      const realTopSize = (this.data.topSize * remScale) / 2;
      const realBottomSize = (this.data.bottomSize * remScale) / 2;
      ...
      this.createSelectorQuery()
        ...
        .boundingClientRect((res) => {
          ...
          this.createSelectorQuery()
            .select(".item-wrap")
            .boundingClientRect((_res) => {
              this.data.itemWrapDom = _res;
